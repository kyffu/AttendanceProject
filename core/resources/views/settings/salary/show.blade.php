...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <h5 class="card-header">Detail Gaji</h5>
                    <div class="card-body">
                        <form action="{{ route('settings.salary.update') }}" method="POST">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="saly" value="{{ Crypt::encryptString($salary->id) }}" />
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="basic-default-name">Deskripsi Gaji</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="description" id="description"
                                        placeholder="Mohon masukkan deskripsi gaji" value="{{ $salary->description }}"
                                        required />
                                </div>
                            </div>
                            <div class="row mb-3">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('settings.salary.destroy') }}">
                            <input type="hidden" name="salyry" value="{{ Crypt::encryptString($salary->id) }}">
                            @csrf
                            @method('DELETE')
